...
import { authenticateToken, checkRole } from '../middlewares/auth.middleawre';
import Book, { BookDocument } from '../models/book.model';

const bookRouter = Router();

// Get all books
bookRouter.get('/', authenticateToken, async (req: Request, res: Response) => {
  try {
...
// Create a book
bookRouter.post('/', authenticateToken, checkRole('CREATOR'), async (req: Request, res: Response) => {
  try {
...
bookRouter.get('/search', authenticateToken, async (req: Request, res: Response) => {
  try {
